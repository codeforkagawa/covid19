<template>
  <div class="About">
    <page-header class="mb-3">{{ $t('当サイトについて') }}</page-header>
    <StaticCard>
      {{
      $t(
      ...
      <a
        ...
        target="_blank"
      >{{ $t('このサイトの運営に貢献していただいた人') }}</a>
      <br />
      <br />
      {{ $t('複製・改変が許されたオープンソースライセンスで公開されている、') }}
      <a
        href="//stopcovid19.metro.tokyo.lg.jp/"
        target="_blank"
      >{{ $t('東京都公式新型コロナウイルス公式対策サイト') }}</a>
      {{ $t('の') }}
      <a
        href="//github.com/tokyo-metropolitan-gov/covid19"
        target="_blank"
      >{{ $t('仕組み') }}</a>
      {{ $t('と') }}
      <a
        href="//covid19-okayama.netlify.com/"
        target="_blank"
      >{{ $t('岡山県新型コロナウイルス感染症非公式対策サイト') }}</a>
      {{ $t('のソースコードを利用しています。') }}
      <br />
      <br />
      {{ $t('また、') }}
      <a href="//www.sakura.ad.jp/" target="_blank">{{ $t('さくらインターネット株式会社様') }}</a>
      {{ $t('よりサーバーの提供を受けております。') }}
      <br />
      <br />
      {{
    ...
    <StaticCard>
      <h3>{{ $t('ブラウザ環境について') }}</h3>
      <p>{{ $t('当サイトは以下の環境でご覧いただくことを推奨いたします。') }}</p>
      <ul>
    ...
    <StaticCard>
      <h3>{{ $t('データについて') }}</h3>
      <i18n tag="p" path="本サイトで公表しているデータは、{catalogWebsite}より誰でも自由にダウンロードが可能です。">
